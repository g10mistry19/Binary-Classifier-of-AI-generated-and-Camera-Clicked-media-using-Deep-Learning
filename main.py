    ...
    if st.button("Check for Deepfake"):
        ...
        if file_type == "Image":
            ...
            st.markdown(
                ...
                <h3>The given {file_type} is: <span class="result"> {result} ''', unsafe_allow_html=True)
            ...
            st.markdown(
                ...
                <h3>The given {file_type} is: <span class="result"> {result}</h3>''', unsafe_allow_html=True)
                # <h3>The given {file_type} is: <span class="result"> {result} </span> with a probability of <span class="result">{pred:.2f}</span></h3>''', unsafe_allow_html=True)
else:
...
The Metrices used to evaluate both models is AUC and Logloss.

EfficientNetB4(AUC Value):   0.8766(DFDC) |  0.9382(FFPP) 
EfficientNetB4(LogLoss):     0.4819(DFDC) |  0.3777(FFPP)
EfficientNetB4ST(AUC Value): 0.8658(DFDC) |  0.9337(FFPP) 
EfficientNetB4ST(LogLoss):   0.5075(DFDC) |  0.3439(FFPP)
